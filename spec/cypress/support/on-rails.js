...
Cypress.Commands.add("appStubs", function (name, options = {}) {
  return cy.app("stub_requests/" + name, options)
})
...
Cypress.on("fail", (err, runnable) => {
  ...
  Cypress.$.ajax({
    url: "/__cypress__/command",
    data: JSON.stringify({name: "log_fail", options: {
      error_message: err.message, runnable_full_title: runnable.fullTitle()
    }}),
    async: false,
    method: "POST"
  })
